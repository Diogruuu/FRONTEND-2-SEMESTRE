...
### 2.1 Componentização
O React organiza a interface em componentes reutilizáveis e independentes. Cada componente encapsula a sua própria lógica, estrutura e, opcionalmente, estilo, o que facilita a manutenção, os testes e a reutilização de código em diferentes partes da aplicação.
...
* **Reutilização de código:** garantida por meio da componentização.
* **Grande comunidade:** vasta documentação que facilita o aprendizado e a resolução de problemas.
* **Curva de aprendizado acessível:** para quem já domina JavaScript.
* **Ampla adoção no mercado:** forte procura por profissionais qualificados.
...
## 5. Ferramentas de Desenvolvimento: Vite
...
* **Fast Refresh (HMR) Ultra Rápido:** A atualização de componentes na tela ao salvar um ficheiro (*Hot Module Replacement*) é quase instantânea e preserva o estado da aplicação.

### Build de Produção Otimizado
* **Poder de esbuild + Rollup:** Durante o desenvolvimento, o Vite usa o `esbuild` (escrito em Go, até 100x mais rápido que bundlers JS tradicionais). Para a build de produção, ele usa o Rollup, gerando bundles altamente otimizados e minificados.
* **Code Splitting Automático:** Divide o código de forma inteligente em pequenos pedaços (*chunks*) para carregar apenas o que o utilizador precisa na tela.

### Suporte Nativo Out-of-the-Box
...
* **Importação de Assets:** Importação direta de imagens, SVGs e ficheiros estáticos com suporte a URLs otimizadas.

### Configuração Leve e Flexível
* **`vite.config.js` Simples:** Diferente do Webpack, onde o ficheiro de configuração costuma ter centenas de linhas complexas, a configuração do Vite é minimalista, legível e baseada em plugins fáceis de estender.
